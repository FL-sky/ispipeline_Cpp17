...
/*
Piecewise Linear (PWL) 曲线 是一种用于描述非线性关系的技术，
通过分段的线性函数来近似一个复杂的曲线。
每段之间由一组控制点或节点定义。
这个解码过程是 ISP 管道中的一个步骤，它将预先定义的非线性校正函数转化为适用于每个像素的线性变换，以实现特定的图像处理效果。
*/
static int Depwl(Frame *frame, const IspPrms *isp_prm)
{
    ...
    FOR_ITER(h, frame->info.height)
    {
        FOR_ITER(w, frame->info.width)
        {
            ...
            raw32_out[pixel_idx] = (raw16_in[pixel_idx] - pwl_prm->x_cood[pwl_idx - 1]) * pwl_prm->slope[pwl_idx] + pwl_prm->y_cood[pwl_idx - 1];
