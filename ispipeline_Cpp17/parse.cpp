...
int ParseIspCfgFile(const std::string cfg_file_path, IspPrms &isp_prm)
{
    ...
    if (raw_type_str == "RAW8")
    {
        isp_prm.info.dt = RawDataTypes::RAW8;
    }
    else if (raw_type_str == "RAW10")
    {
    ...
    if ((j_root["lsc"]["mesh_width_nums"] != kLscMeshPointHNums) || (j_root["lsc"]["mesh_height_nums"] != kLscMeshPointVNums))
    {
